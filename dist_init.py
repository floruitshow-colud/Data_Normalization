...
def split_parquet_by_count(config, parquet_list, node_id, num_proc_per_node, num_total_proc):
    ...
    assert len (parquet _list) >=num_parquet
    parquet_list = parquet_ listIrange_start: range_end if num_parquet < num_total_proc:
        print(f"[WARNI]parquet(‹num_parquet)) is less than num_total proc({num_total_proc}),some proc tasks will skip.")
range_total = ［0 for_in
range(num_total_proc)]
range_count =
np.zeros(num_total_proc)
for par in parquet _list:
num = 1
curr_min_idx=
range_count.argmin()
range_total[curr_min_idxl.append(par
）＃贪心策略 每次将新parquet 填入当前样本负载最小的进程
range_count[curr_min_idx] +=
num
print(f'LINFOl range_count:
{range_count})
range_node = range_totalinode_id
* num_proc_per_node： （node_id + 1）*

def wait_complete process_list:
List Imp.Process], rank: int):
complete_flag = False
while not complete_flag:
time.sleep (5)
for p in process_list:
if not p.is_alivel and
p.exitcode!= 0:
print| [WARNING] process
failed)
# raise
ValueError(f"process exitcode:
{p.exitcode) )
complete_flag = any(lp.is_alivel)
for p in process_list.) is False
print(f"rank:frank} task complete.")
6002
...
parquet_list +=
Los.path.join(path,file) for file in mox.file list _directory(path,recursive=
True, skip_dir=False) if not file.endswith('.xlsx) and not
file.endswith.csv) ]
# parquet _list +=
[os.path.join(path, file) for file in os.listdir(path) if
file.endswith(file_type)]
printf"(path} parquet _list is {len(parquet_list)}")
return parquet_list
if _name_=='_main_:
parser =
argparse. ArgumentParser)
parser.add.
Largument('--config_file, type=str, default='config_SJZG.yaml')
args, unparsed =
parser.parse_known_args)
config =
ConfigUtils.parse_config(args)

os.makedirs/config.tmp_tar_local_pat
h, exist_ok= True)
...
os.makedirs/config.tmp_img_local_pa th, exist_ok=True)
os.makedirs/config.init_parquet_local
_path, exist_ok=True) os.makedirs(/cache/txt,
exist_ok=True)
